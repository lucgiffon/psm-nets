...
if __name__ == "__main__":
    ...
    expe_path_palminized_only_mask = "2020/03/5_6_finetune_palminized_only_mask"  # case with no scaling
    ...
    sparsity_factors = sorted(set(df_palminized_only_mask["--sparsity-factor"]))
    nb_factors = set(df_palminized_only_mask["--nb-factor"])
    ...
    df_palminized_only_mask = df_palminized_only_mask.apply(pd.to_numeric, errors='coerce')
    ...
    for dataname in dataset:
        df_data_only_mask = df_palminized_only_mask[df_palminized_only_mask[dataset[dataname]] == 1]
        ...
        for task in tasks:
            ...
            # only mask finetune
            #############################
            if not(task == "compression_rate" or task == "nb_flop" or task == "nb_param"):
                for i, sp_fac in enumerate(sparsity_factors):
                    df_sparsity = df_data_only_mask[df_data_only_mask["--sparsity-factor"] == sp_fac]
                    for j, hierarchical_value in enumerate([1, 0]):
                        hierarchical_str = " H" if hierarchical_value == 1 else ""
                        df_hier = df_sparsity[df_sparsity["--hierarchical"] == hierarchical_value]
                        finetune_score_values = df_hier.sort_values("--nb-factor", na_position="last")[task].values
