...
if __name__ == "__main__":
    ...
    results_path = "2020/04/0_0_finetune_faust_sp_6_8_fac_2"
                ...
                for i, sp_fac_palm in enumerate(sparsy_factors_palm):
                    ...
                    for hierarchical_value in hierarchical_values:
                        ...
                        for keep_last_layer in [1, 0]:
                            df_keep = df_data_palminized_hierarchical[df_data_palminized_hierarchical["keep-last-layer"] == keep_last_layer]
                            df_keep = df_keep.apply(pd.to_numeric, errors='coerce')
                            val = df_keep.sort_values("nb-factor", na_position="last")[task].values
                            hls_str = "hsl({}, {}%, {}%)".format(hue_by_sparsity[sp_fac_palm], saturation_by_hier[hierarchical_value], lum_by_keep[keep_last_layer])
                            str_trace = ('Palm {} keep {}' + hierarchical_str).format(sp_fac_palm, keep_last_layer)
