...
def get_palm_results():
    ...
    results_path = "2020/11/12_13_finetune_sparse_facto_plus_2steps"
...
if __name__ == "__main__":
    ...
    df_palm = df_palm[df_palm["model"] == "resnet20"]
    ...
    for dataname in datasets:
        ...
        for modelname in df_model_values:
            df_model_palm_base = df_data_palm[df_data_palm["model"] == modelname]
            ...
            df_model_palm = df_model_palm_base
            # df_model_palm = df_model_palm_base[df_model_palm_base["multi-step"] == False]

            palm_algo = "PSM"
            groupby_col = ['hierarchical', 'nb-factor', 'sparsity-factor', 'keep-first-layer', 'only-mask', 'multi-step']
            ...
            for names, group in gp_by:
                ...
                multi_step = bool(dct_group_by_vals["multi-step"])
                ...
                name_trace = f"{palm_algo} Q={nb_factor} K={sparsity_factor} {'multi' if multi_step else ''}"
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        marker=dict(
                            color=dct_colors[f"PALM K={sparsity_factor}"] if multi_step is False else 'black',
            ...
            fig.update_layout(
                # title=title,
                xaxis_title="# Param. in Conv. and Dense layers",
