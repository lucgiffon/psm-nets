...
if __name__ == "__main__":
    ...
    results_path = "2021/01/13_14_compression_baselines_grid_lr"
    ...
    root_output_dir = pathlib.Path("/home/luc/PycharmProjects/palmnet/parameters/2021/01")
    ...
    for dataname in datasets:
        ...
        for modelname in df_model_values:
            ...
            # magnitude
            for magnitude_str in ["magnitude", "magnitude_hard"]:
                df_magnitude = df_model[df_model["compression"] == magnitude_str]
                dct_results[dataname][modelname][magnitude_str] = dict()
                ...
                for sparsity in sparsity_values:
                    ...
                    dct_results[dataname][modelname][magnitude_str][float(sparsity)] = float(lr_values[best_score_indice])
            ...
            df_multidense = df_model[df_model["compression"] == "multidense"]
            dct_results[dataname][modelname]["multidense"] = dict()
            nb_factor_values = set(df_multidense["nb-fac"].values)
            for nb_fac in nb_factor_values:
                df_nb_fac = df_multidense[df_multidense["nb-fac"] == nb_fac]
                sparsity_values = set(df_nb_fac["final-sparsity"].values)
                dct_results[dataname][modelname]["multidense"][int(nb_fac)] = dict()
                for sparsity in sparsity_values:
                    df_sparsity = df_nb_fac[df_nb_fac["final-sparsity"] == sparsity]
                    dct_results[dataname][modelname]["multidense"][int(nb_fac)][float(sparsity)] = dict()
                    for double_epochs in [True, False]:
                        df_epochs = df_sparsity[df_sparsity["double-epochs-pruning"] == double_epochs]
                        if len(df_epochs) == 0:
                            continue
                        lr_values = df_epochs["learning-rate"].values
                        score_values = df_epochs["finetuned-score-val"].values
                        best_score_indice = np.argmax(score_values)
                        dct_results[dataname][modelname]["multidense"][int(nb_fac)][float(sparsity)][double_epochs] = float(lr_values[best_score_indice])
