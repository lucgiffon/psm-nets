...
if __name__ == "__main__":
    ...
    model = Sequential()
    model.add(RandomSparseFactorisationConv2DDensify(input_shape=input_shape, filters=nb_filter, kernel_size=kernel_size, padding=padding, sparsity_factor=sparsity_factor, nb_sparse_factors=sparse_factors))
    model.add(RandomSparseFactorisationConv2DDensify(filters=nb_filter*2, kernel_size=kernel_size, padding=padding, sparsity_factor=sparsity_factor, nb_sparse_factors=sparse_factors))
    model.add(RandomSparseFactorisationConv2DDensify(filters=nb_filter*4, kernel_size=kernel_size, padding=padding, sparsity_factor=sparsity_factor, nb_sparse_factors=sparse_factors))
    model.add(Flatten())
    ...
    Palminizable.count_model_param_and_flops_(model)
