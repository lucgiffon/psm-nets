...
def plot_decision_bounds(names, classifiers):
    ...
    datasets = [make_moons(n_samples=n_samples, noise=0.5, random_state=0),
                make_circles(n_samples=n_samples, noise=0.40, factor=0.2, random_state=1),
                linearly_separable
                ]
    ...
    for ds_cnt, ds in enumerate(datasets):
        ...
        for name, clf in zip(names, classifiers):
            ...
            if hasattr(clf, "decision_function"):
                ...
                Z = np.rint(Z) # not have shaded areas
...
names = ["Arbre seul", "Forêt (100 arbres)"]
