...
Usage:
    script.py (faust|palm) [--tucker] --input-dir path [--multi-step] [--lr-configuration-file path] [-h] [-v|-vv] [--seed int] [--only-dense] [--logrange-clr] [--train-val-split float] [--batchnorm] [--keep-last-layer] [--keep-first-layer] [--lr float] [--use-clr policy] [--min-lr float --max-lr float] [--epoch-step-size int] [--nb-epoch int] [--only-mask] [--tb] (--mnist|--svhn|--cifar10|--cifar100|--test-data) [--cifar100-resnet50|--cifar100-resnet20|--cifar100-resnet50-new|--cifar100-resnet20-new|--mnist-500|--mnist-lenet|--test-model|--cifar10-vgg19|--cifar100-vgg19|--svhn-vgg19] --sparsity-factor=int [--nb-iteration-palm=int] [--delta-threshold=float] [--hierarchical] [--nb-factor=int] [--max-cum-batch-size int] [--batch-size int] [--nb-epochs int] [--full-model-approx] [--only-one-batch] [--activations]
...
Options:
  ...
  --lr-configuration-file path          Tell the path to the lr configuration
  --multi-step                          Tells if it is a multi step compression
    ...
    for idx_layer, compressed_layer in enumerate(new_model.layers):
        if any(isinstance(compressed_layer, _class) for _class in (Dense, Conv2D, SparseFactorisationDense, SparseFactorisationConv2D, TuckerSparseFactoLayerConv)):
            ...
            if not paraman["--tucker"]:
                try:
                    sparse_factorization = dct_name_compression[base_layer.name]
                except KeyError:
                    sparse_factorization = next(v for k, v in dct_name_compression.items() if k.startswith(base_layer.name + "_-_"))
