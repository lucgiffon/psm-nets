...
Usage:
    script.py [-h] [-v|-vv] [--tucker] [--nb-factor=int] --sparsity-factor=int [--nb-iteration-palm=int] [--tol=float] [--hierarchical] (--mnist|--svhn|--cifar10|--cifar100|--test-data) [--cifar100-resnet50|--cifar100-resnet20|--mnist-500|--mnist-lenet|--test-model|--cifar10-vgg19|--cifar100-vgg19|--svhn-vgg19]
...
Palm-Specifc options:
  ...
  --tucker                              Apply first tucker decomposition to layers then factorize the obtained convolutional layers.
"""
import logging
import pickle
import sys
...
def compress_model(base_model):
    ...
    if paraman["--tucker"]:
        layer_replacer = LayerReplacerSparseFactoTuckerFaust(sparse_factorizer=faustizer, path_checkpoint_file=paraman["output_file_modelprinter"])
    else:
