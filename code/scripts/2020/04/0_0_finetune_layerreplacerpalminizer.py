...
Usage:
    script.py --input-dir path [-h] [-v|-vv] [--seed int] [--train-val-split float] [--batchnorm] [--keep-last-layer] [--lr float] [--use-clr policy] [--min-lr float --max-lr float] [--epoch-step-size int] [--nb-epoch int] [--only-mask] [--tb] (--mnist|--svhn|--cifar10|--cifar100|--test-data) [--cifar100-resnet50|--cifar100-resnet20|--mnist-500|--mnist-lenet|--test-model|--cifar10-vgg19|--cifar100-vgg19|--svhn-vgg19] --sparsity-factor=int [--nb-iteration-palm=int] [--delta-threshold=float] [--hierarchical] [--nb-factor=int]
...
def compress_and_evaluate_model(base_model, model_compilation_params, x_test, y_test):
    layer_replacer = LayerReplacerPalm(keep_last_layer=paraman["--keep-last-layer"],
                                       only_mask=paraman["--only-mask"], path_checkpoint_file=paraman["input_model_path"],
                                       intertwine_batchnorm=paraman["--batchnorm"])

    with open(paraman["input_model_path"], "rb") as rb_of:
        mypalminizedmodel = pickle.load(rb_of)
    dct_name_facto = mypalminizedmodel.sparsely_factorized_layers
    layer_replacer.dct_name_compression = dct_name_facto
