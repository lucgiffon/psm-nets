...
Usage:
    ...
    script.py multidense [-h] [-v|-vv] [--lr-configuration-file path] [--seed int] [--nb-factor int] [--final-sparsity float] [--double-epochs-pruning] [--batch-size int] [--train-val-split float] [--only-dense] [--keep-last-layer] [--keep-first-layer] [--lr float] [--nb-epoch int] [--use-clr] [--min-lr float] [--max-lr float] [--epoch-step-size int] (--mnist|--svhn|--cifar10|--cifar100|--test-data) [--cifar100-resnet50-new|--cifar100-resnet20-new|--cifar100-resnet50|--cifar100-resnet20|--mnist-500|--mnist-lenet|--test-model|--cifar10-vgg19|--cifar100-vgg19|--svhn-vgg19]
...
Multidense options:
    --double-epochs-pruning             Double the number of epochs for pruning
...
def get_params_optimizer():
    ...
    elif paraman["magnitude_hard"]:
        str_method = "magnitude_hard"
    elif paraman["random"]:
        str_method = "random"
    elif paraman["dummy"]:
        str_method = "dummy"
    elif paraman["multidense"]:
        str_method = "multidense"
    else:
    ...
    if paraman["--lr-configuration-file"] is not None:
        ...
        params_optimizer["lr"] = load_function_lr(dct_res_new=dct_config_lr,
                                                  ...
                                                  double_epochs_value=paraman["--double-epochs-pruning"],
                                                  nb_fac_multidense=paraman["--nb-factor"],
                                                  sparsity_value_multidense=paraman["--final-sparsity"])
    else:
...
def define_callbacks(param_train_dataset, x_train):
    ...
    if paraman["magnitude"] or (paraman["multidense"] and paraman["--final-sparsity"] != 0):
        call_backs.append(sparsity.UpdatePruningStep())

    dct_results = {
        "actual-batch-size": param_train_dataset.batch_size,
        "actual-nb-epochs": (param_train_dataset.epochs if paraman["--nb-epoch"] is None else paraman["--nb-epoch"]) * (2 if paraman["--double-epochs-pruning"] else 1),
...
def get_and_evaluate_base_model(model_compilation_params, x_train, y_train, x_val, y_val, x_test, y_test):
    ...
    if paraman["magnitude"] or paraman["multidense"]:
...
def compress_and_evaluate_model(base_model, model_compilation_params, param_train_dataset, x_train, y_train, x_val, y_val, x_test, y_test):
    if paraman["tucker"]:
        ...
        # base_model = translate_keras_to_tf_model(base_model)
        layer_replacer = LayerReplacerMagnitudePruning(final_sparsity=paraman["--final-sparsity"], hard=True,
                                                       keep_last_layer=paraman["--keep-last-layer"], keep_first_layer=paraman["--keep-first-layer"],
                                                       only_dense=paraman["--only-dense"])
    elif paraman["multidense"]:
        epochs = param_train_dataset.epochs if paraman["--nb-epoch"] is None else paraman["--nb-epoch"]

        if paraman["--double-epochs-pruning"]:
            start_step = np.ceil(1.0 * x_train.shape[0] / param_train_dataset.batch_size).astype(np.int32) * epochs
            end_step = np.ceil(1.0 * x_train.shape[0] / param_train_dataset.batch_size).astype(np.int32) * (epochs *2)
        else:
            start_step = 0
            end_step = np.ceil(1.0 * x_train.shape[0] / param_train_dataset.batch_size).astype(np.int32) * epochs

        layer_replacer = LayerReplacerMulti(nb_factors=paraman["--nb-factor"],
                                            final_sparsity=paraman["--final-sparsity"], end_step=end_step, begin_step=start_step,
...
def fit_new_model(new_model, param_train_dataset, init_nb_epoch, call_backs, x_train, y_train, x_test, y_test, x_val, y_val):
    ...
    new_model.fit(param_train_dataset.image_data_generator.flow(x_train, y_train,
                  ...
                  epochs=((param_train_dataset.epochs if paraman["--nb-epoch"] is None else paraman["--nb-epoch"]) * (2 if paraman["--double-epochs-pruning"] else 1)) - init_nb_epoch,
