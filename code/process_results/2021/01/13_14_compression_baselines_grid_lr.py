...
def get_df_from_expe_path(expe_path):
    ...
    # before these expe, resnet results are invalid

    lst_expepath_special = [
    ...
    if expe_path in lst_expepath_special:
    ...
    #         and expe_path != "2020/09/12_13_compression_baselines_magnitude_hard_find_lr":
...
if __name__ == "__main__":
    ...
    experiment_name = "2021/01/13_14_compression_baselines_grid_lr"
    ...
    lst_paths_finetune = [
        ...
        "2020/09/12_13_compression_baselines_magnitude_hard_find_lr",
        "2021/01/13_14_compression_multidense_find_lr"
    ]
    ...
    for idx, (_, row) in enumerate(df_tucker_tt.iterrows()):
        ...
        elif row["magnitude_hard"] is True:
            dct_attributes["compression"].append("magnitude_hard")
        elif row["random"] is True:
            dct_attributes["compression"].append("random")
        elif row["multidense"] is True:
            dct_attributes["compression"].append("multidense")
        else:
        ...
        #multidense informations
        dct_attributes["double-epochs-pruning"].append(bool(row["--double-epochs-pruning"]) if not np.isnan(row["--double-epochs-pruning"]) else np.nan)
