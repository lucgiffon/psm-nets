...
if __name__ == "__main__":
    ...
    expe_path = "2020/11/12_13_finetune_palm_not_log_highK_2step_2ndround_find_lr"

    lst_path_finetune = [
        "2020/11/12_13_finetune_palm_not_log_highK_2step_2ndround_find_lr"
    ]
    ...
    for idx, (_, row) in enumerate(df_finetune.iterrows()):
        ...
        # palm act info
        dct_attributes["batch-size"].append(int(row["--batch-size"]) if not np.isnan(row["--batch-size"]) else None)
        dct_attributes["max-cum-batch-size"].append(int(row["--max-cum-batch-size"]) if not np.isnan(row["--max-cum-batch-size"]) else None)
        dct_attributes["nb-epochs"].append(int(row["--nb-epochs"]) if not np.isnan(row["--nb-epochs"]) else None)
