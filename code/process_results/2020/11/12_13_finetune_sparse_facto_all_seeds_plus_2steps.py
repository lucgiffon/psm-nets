...
if __name__ == "__main__":
    ...
    expe_path = "2020/11/12_13_finetune_sparse_facto_plus_2steps"

    lst_path_finetune = [
        "2020/07/11_12_finetune_sparse_facto_fix_replicates",
        "2020/07/11_12_finetune_sparse_factro_fix_only_mask",
        "2020/11/12_13_finetune_palm_not_log_highK_2step_2ndround"
    ]
    ...
    for idx, (_, row) in enumerate(df_finetune.iterrows()):
        ...
        # 2 step informations
        dct_attributes["multi-step"].append(bool(row["--multi-step"]) if not np.isnan(row["--multi-step"]) else False)
